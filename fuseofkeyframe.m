...
%        for ll=1:(c-3)
%         G=f(kk:(kk+2),ll:(ll+2));
%             if(log1p(sum(sum(G))/double(9)) >= 0 )
%                m(kk,ll)=(log1p(sum(sum(G))/double(9)));
%             else
%             m(kk,ll)=0;
%             end
%                
% px=real(O(kk:(kk+2),ll:(ll+2)));
% py=real(O1(kk:(kk+2),ll:(ll+2)));
%       T=cov(px,py,0);
%       Ei=eig(T);
%        x1=Ei(1);
%      y1=Ei(2);
...
%      su=(x1-y1);
%      ad=(x1+y1);
%      xyz(kk,ll)=round(su.*su,2);
%      xy(kk,ll)=round(ad.*ad,4);
%       if(ad > 0 && ((xyz(kk,ll)/xy(kk,ll)~=1)) && ((xyz(kk,ll)/xy(kk,ll)>0)) && ~isnan((xyz(kk,ll)/xy(kk,ll))))
%      pic(kk,ll)=(xyz(kk,ll)/xy(kk,ll));
%       else
%        pic(kk,ll)=0;   
%        
%       end
...
%       fuse=(mag+pic);
      ...
      %fuse=double(fuse)/double(max(max(fuse)));         
                ...
                filename = fullfile('C:\Users\SAURADIP\Documents\MATLAB\video frame multimodal\new race fused 9\fused frame', sprintf('fusedraceframe_%05d.JPG', g));   %Writing the keyframes
imwrite(fuse, filename);
           end
   end
